...
test("autosave is disabled by default", () => {
	assert.equal(getDefaultPhotonAutosaveEnabled(), false);
});
...
test("fingerprints are workspace-aware so branches keep independent dirty state", () => {
	...
	const mainFingerprint = getPhotonStudioFingerprint(
		document,
		{},
		{},
		{
	...
	const featureFingerprint = getPhotonStudioFingerprint(
		document,
		{},
		{},
		{
	...
	assert.equal(
		resolvePhotonStudioWorkspaceKey(mainWorkspace),
		"profile-a:main:head",
	);
});

test("workspace head changes do not invalidate the studio fingerprint", () => {
	const previousHeadFingerprint = getPhotonStudioFingerprint(
		document,
		{},
		{},
		{
	...
	const updatedHeadFingerprint = getPhotonStudioFingerprint(
		document,
		{},
		{},
		{
...
test("draft storage keys are namespaced by workspace identity", () => {
	const draftKeyA = resolvePhotonStudioDraftStorageKey({
		baseKey: "photon:draft",
		workspace: {
			ref: {
				profileId: "profile-a",
				branch: "main",
	...
	const draftKeyB = resolvePhotonStudioDraftStorageKey({
		baseKey: "photon:draft",
		workspace: {
			ref: {
				profileId: "profile-a",
	...
	assert.equal(draftKeyA, "photon:draft:profile-a:main:head");
	assert.equal(
		draftKeyB,
		"photon:draft:profile-a:feature/redesign:head",
	);
...
test("save payloads keep optimistic concurrency separate from workspace identity", () => {
	const autosavePayload = createPhotonStudioSavePayload({
		workspace: {
			ref: {
				profileId: "profile-a",
				branch: "main",
	...
	const manualPayload = createPhotonStudioSavePayload({
		workspace: {
			ref: {
				profileId: "profile-a",
				branch: "main",
...
test("site setting change payloads receive the updated site snapshot and workspace metadata", () => {
	const context = createPhotonStudioSiteSettingChangeContext({
