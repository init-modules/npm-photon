...
import { createWebsiteBuilderRegistry } from "../helpers/document";
import { createWebsiteBuilderStore } from "./website-builder-store";
...
const createStore = () =>
	createWebsiteBuilderStore({
		...
		registry: createWebsiteBuilderRegistry([]),
		workspace: {
			ref: {
				profileId: "profile-a",
				branch: "main",
...
test("readonly revision workspaces reject editor mutations", () => {
	const store = createWebsiteBuilderStore({
