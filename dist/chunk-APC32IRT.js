...
var usePhotonBlockActiveState = (blockId) => {
  const ctx = usePhoton();
  return useMemo(() => {
    const block = findPhotonPublicBlock(ctx.document.blocks, blockId) ?? findPhotonPublicComponentSourceBlock(ctx.site, blockId);
    const definition = block ? ctx.registry.getDefinition(block.module, block.type) : void 0;
