...
var collectPhotonInteractionSurfaces = (entries) => entries.flatMap(
  (entry) => isPhotonInstallableKit(entry) ? entry.interactionSurfaces ?? [] : []
);
var collectPhotonInteractionActions = (entries) => entries.flatMap(
  (entry) => isPhotonInstallableKit(entry) ? entry.interactionActions ?? [] : []
);
var collectPhotonInteractionGuards = (entries) => entries.flatMap(
  (entry) => isPhotonInstallableKit(entry) ? entry.interactionGuards ?? [] : []
);
var collectPhotonInteractionGuardEvaluators = (entries) => Object.assign(
  {},
  ...entries.map(
    (entry) => isPhotonInstallableKit(entry) ? entry.interactionGuardEvaluators ?? {} : {}
  )
);
var collectPhotonInteractionPolicies = (entries) => entries.flatMap(
  (entry) => isPhotonInstallableKit(entry) ? entry.interactionPolicies ?? [] : []
);
var collectPhotonConditionDefinitions = (entries) => entries.flatMap(
  (entry) => isPhotonInstallableKit(entry) ? entry.conditionDefinitions ?? [] : []
);
var collectPhotonConditionEvaluators = (entries) => Object.assign(
  {},
  ...entries.map(
    (entry) => isPhotonInstallableKit(entry) ? entry.conditionEvaluators ?? {} : {}
  )
);
var collectPhotonSiteDataSchemas = (entries) => entries.flatMap(
  (entry) => isPhotonInstallableKit(entry) ? entry.siteDataSchemas ?? [] : []
);
var collectPhotonRouteContextFields = (entries) => entries.flatMap(
  (entry) => isPhotonInstallableKit(entry) ? entry.routeContextFields ?? [] : []
);
var collectPhotonFormSchemas = (entries) => entries.flatMap(
  (entry) => isPhotonInstallableKit(entry) ? entry.formSchemas ?? [] : []
);
