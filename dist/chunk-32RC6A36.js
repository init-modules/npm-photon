...
var getPhotonSiteColorScheme = (id) => siteColorSchemeMap.get(id);

// src/modules/system/site/site-design-presets.ts
var PHOTON_DEFAULT_SITE_DESIGN_PRESET_ID = "aurora-current";
...
var getPhotonSiteDesignPreset = (id) => siteDesignPresetMap.get(id);

// src/helpers/site-design.ts
var PHOTON_SITE_DESIGN_TOKEN_KEYS = [
  "bodyFontFamily",
  "headingFontFamily",
  "backgroundColor",
  "surfaceColor",
...
var PHOTON_SITE_DESIGN_COLOR_TOKEN_KEYS = [
  "backgroundColor",
  "surfaceColor",
  "textColor",
  "mutedTextColor",
...
var PHOTON_SITE_DESIGN_FALLBACK_DEFAULTS = {
...
var matchesPhotonSiteDesignDefaults = (left, right) => PHOTON_SITE_DESIGN_TOKEN_KEYS.every(
...
var pickSiteDesignTokens = (value) => PHOTON_SITE_DESIGN_TOKEN_KEYS.reduce(
  (result, key) => {
...
var createResolvedSettings = ({
  ...
  const preset = presetId ? getPhotonSiteDesignPreset(presetId) : void 0;
  const colorScheme = colorSchemeId ? getPhotonSiteColorScheme(colorSchemeId) : void 0;
  return {
    ...PHOTON_SITE_DESIGN_DEFAULTS,
    ...preset?.designTokens ?? {},
...
var createPhotonSiteDesignSettings = ({
  presetId,
  colorSchemeId,
  componentVariants,
  overrides
...
var isPhotonFramelessPreset = (presetId) => typeof presetId === "string" && PHOTON_FRAMELESS_PRESET_IDS.has(presetId);
var isPhotonFramelessSiteDesign = (value) => isPhotonFramelessPreset(
  resolvePhotonSiteDesignSettings(value).presetId
);
var applyPhotonSiteDesignPreset = (value, presetId) => {
...
var isPhotonSiteDesignPresetApplied = (settings, presetId) => {
  const preset = getPhotonSiteDesignPreset(presetId);
  if (!preset) {
    return false;
  }
...
var hasPhotonSiteDesignPresetCustomization = (settings, presetId) => {
  const preset = getPhotonSiteDesignPreset(presetId);
  return settings.presetId === preset?.id && !isPhotonSiteDesignPresetApplied(settings, presetId);
};
var applyPhotonSiteColorScheme = (value, colorSchemeId) => {
  const currentSettings = resolvePhotonSiteDesignSettings(value);
...
var resolvePhotonSiteDesignSettings = (value) => {
  ...
  const explicitPreset = presetId ? getPhotonSiteDesignPreset(presetId) : void 0;
  const explicitColorScheme = colorSchemeId ? getPhotonSiteColorScheme(colorSchemeId) : void 0;
  const isFallbackDesign = !explicitPreset && !explicitColorScheme && !hasComponentVariantOverrides && matchesPhotonSiteDesignDefaults(
