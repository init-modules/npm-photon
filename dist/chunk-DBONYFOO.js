...
var insertManyIntoList = (blocks, newBlocks, index) => {
  const nextBlocks = [...blocks];
  const safeIndex = Math.min(Math.max(index, 0), nextBlocks.length);
...
var insertManyIntoListTree = (blocks, listId, newBlocks, index) => {
  if (listId === PHOTON_ROOT_LIST_ID) {
    return {
...
var insertPhotonBlocksInDocument = (document, listId, blocks, index) => {
...
var replaceBlockWithBlocks = (blocks, blockId, replacementBlocks) => {
  const targetIndex = blocks.findIndex((block) => block.id === blockId);
  if (targetIndex >= 0) {
    return {
      blocks: [
...
var replacePhotonBlockWithBlocksInDocument = (document, blockId, replacementBlocks) => {
